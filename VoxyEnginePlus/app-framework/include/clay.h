...
CLAY__TYPEDEF(
    Clay_LayoutElementHashMapItem, struct { // todo get this struct into a single cache line
        Clay_BoundingBox boundingBox;
...
Clay__DebugElementConfigTypeLabelConfig Clay__DebugGetElementConfigTypeLabel(Clay__ElementConfigType type)
{
    ...
    {
    case CLAY__ELEMENT_CONFIG_TYPE_RECTANGLE:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Rectangle"), {243, 134, 48, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_TEXT:
...
void Clay_UpdateScrollContainers(bool enableDragScrolling, Clay_Vector2 scrollDelta, float deltaTime)
{
    ...
    for (int32_t i = 0; i < Clay__scrollContainerDatas.length; i++)
    {
        ...
        for (int32_t j = 0; j < Clay__pointerOverIds.length; ++j)
